export const SAVE_AFFIRMATION_REPORT = 'save_affirmation_report';
export const SAVE_AGGRESSION_REPORT = 'save_aggression_report';
export const RESET_AGGRESSION_REPORT = 'reset_aggression_report';
export const RESET_AFFIRMATION_REPORT = 'reset_affirmation_report';
export const ADD_AGGRESSION_REPORT = 'add_aggression_report';
export const ADD_AFFIRMATION_REPORT = 'add_affirmation_report';
export const GET_AGGRESSION_REPORTS = 'get_aggression_reports';
export const DELETE_REPORT = 'delete_report';
export const SAVE_SURVEY_A = 'save_survey_a';
export const SAVE_SURVEY_B = 'save_survey_b';
export const SAVE_SURVEY_C = 'save_survey_c';
export const SAVE_SURVEY_D = 'save_survey_d';
export const SAVE_SURVEY_E = 'save_survey_e';
export const SAVE_SURVEY_L = 'save_survey_l';
export const RESET_SURVEY_A = 'reset_survey_a';
export const RESET_SURVEY_B = 'reset_survey_b';
export const RESET_SURVEY_C = 'reset_survey_c';
export const RESET_SURVEY_D = 'reset_survey_d';
export const RESET_SURVEY_E = 'reset_survey_e';
export const RESET_SURVEY_L = 'reset_survey_l';
export const ADD_SURVEY = 'add_survey';
export const SAVE_INTRO_SURVEY = 'save_intro_survey';
export const ADD_PENDING_SURVEY = 'add_pending_survey';
export const UPDATE_PENDING_SURVEY = 'update_pending_survey';
export const ACTIVATE_SURVEY = 'activate_survey';
export const DEACTIVATE_SURVEY = 'deactivate_survey';
export const COMPLETE_INTRO_SURVEY = 'complete_intro_survey';
export const GET_SURVEY_A = 'get_survey_a';
export const GET_SURVEY_B = 'get_survey_b';
export const GET_SURVEY_C = 'get_survey_c';
export const SAVE_CONSENT = 'save_consent';
export const GET_CONSENT = 'get_consent';
export const SAVE_SLIDESHOW = 'save_slideshow';
export const RESET_APP = 'reset_app';
export const SAVE_POST_MEASURE_SURVEY = 'save_post_measure_survey';
export const SAVE_DEVICE = 'save_device';
export const SAVE_USER = 'save_user';
export const GET_USER = 'get_user';
export const GET_USER_SUCCESS = 'get_user_success';
export const GET_USER_ERROR = 'get_user_error';